...
uses map to 
return an array of names reversed.
...
const getNames = (arr) => {
  ...
  return arr.map(person => {
    const nameArr = Array.from(person.name).reverse()
    //.from changed string to charector array
    //.reverse flipped the charectors
    return nameArr.join('')
    //.join('') puts the chars together without space
  });
};
// Ref. Helped get solution from Joe Ivans
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
